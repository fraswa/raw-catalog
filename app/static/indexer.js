...
$('startScan').onclick=async()=>{clearError();$('startScan').disabled=true;try{await api('/api/scan',{method:'POST',body:JSON.stringify({force:$('force').checked,skip_previews:$('skipPreviews').checked,skip_imported:$('skipImported').checked,skip_post_stat:$('skipPostStat').checked,parallelism:Number($('parallelism').value)})});clearTimeout(timer);await poll();}catch(err){showError(err);$('startScan').disabled=false;}};
